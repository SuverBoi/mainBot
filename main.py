...
Вступ: Джеймс Вільямсон завжди мріяв дізнатися більше про загублене місто, яке легендарно вважалося скарбницею древніх знань. Він досліджував довгий час і врешті-решт виявив сліди, що вказували на те, що місто може бути реальним. Джеймс вирішив покинути усе і вирушити у відправлення на пошуки Загубленого Міста разом зі своєю командою.''', reply_markup=arc_story())
    elif msg.text == "Продовжити🚀":
        bot.send_message(cid, '''Частина 1: Вибір шляху 

Джеймс та його команда опиняються перед розгалуженням у лісі. Є два шляхи, які вони можуть обрати: через глибокий ліс або піднятися на гору для кращого огляду.''', reply_markup=arc_story_var1())

    elif msg.text == "🌳 Пройти через глибокий ліс.":
        bot.send_message(cid, '''Варіант 1: Глибокий ліс

Джеймс та його команда вирішують пройти через глибокий ліс. Вони потрапляють у гущавину, де їх оточують високі дерева. Ліс виявляється заплутаним, і вони заблукають. Подальший пошук артефакту ускладнюється через збірність місцевості.''', reply_markup=arc_story_var2())
    elif msg.text == "🪨 Піднятися на гору для кращого огляду.":
        bot.send_message(cid, '''Варіант 2: Гора
W   
Джеймс та його команда вирішують піднятися на гору для кращого огляду. Зверху вони бачать прекрасний краєвид, але ніякого міста. Але їх увагу привертає загадковий символ на скелі. Це може бути ключем до того, де знаходиться місто.''', reply_markup=arc_story_var3())
    elif msg.text == "👣 Досліджувати руїни.":
        bot.send_message(cid, '''Вони розбивають табір і намагаються розібратися, як пройти крізь ліс. Нарешті, після декількох днів блукання, вони знаходять виходи з лісу. Вони потрапляють на відкрите поле, де бачать давні руїни.''', reply_markup=arc_story_var4())
    elif msg.text == "👁️ Досліджувати символ на скелі.":
        bot.send_message(cid, '''Вони досліджують символ на скелі і розуміють, що він вказує на підземний вхід. Вони спускаються вниз і опиняються в загадковому підземеллі. Вони виявляють артефакт, який виділяється загадковим сяйвом.''', reply_markup=arc_story_var5())
    elif msg.text == "🚀 Вирушити назад до табору.":
        bot.send_message(cid, '''Вони розбивають табір і повертаються до свого табору, де вирішують переглянути свої збірки та зробити нові плани. Зробіть за них вибір: ''', reply_markup=arc_story_var1())
    elif msg.text == "🗡️ Вступити в бій зі сторожами.":
        bot.send_message(cid, '''Вступити в бій зі сторожами

Джеймс та його команда вирішують вступити в бій зі сторожами руїн. Бій стає жорстоким, але вони зумівають перемогти сторожів. Після бою, вони знаходять артефакт, який лежить на підігрітій плиті. Здавалося б, вони здобули перемогу, але раптово починається землетрус, і вони повинні швидко вирватися з руїн, тримаючи артефакт.''', reply_markup=arc_story_var6())
    elif msg.text == "🏃‍♀️ Постаратися уникнути сторожів.":
        bot.send_message(cid, '''Джеймс та його команда намагаються уникнути зіткнення зі сторожами руїн. Вони обходять сторожів і вирішують обійти руїни. Проте вони помічають загадковий артефакт на підігрітій плиті.''', reply_markup=arc_story_var7())

    elif msg.text == "👁️ Досліджувати артефакт детальніше.":
       bot.send_message(cid, ''' Досліджувати артефакт детальніше

Джеймс та його команда вирішують дослідити артефакт детальніше. Вони наближаються до артефакта, і відразу відчувають загадкову енергію, яка випромінює з нього. Раптово артефакт активізується, і перед ними з'являється голограма, яка вказує на напрямок подальшого дослідження.''', reply_markup=arc_story_var8())
    elif msg.text == "🏃‍♂️ Піти з підземелля.":
        bot.send_message(cid, ''' Піти з підземелля

Відчуваючи незвичну енергію артефакта, Джеймс та його команда вирішують залишити підземелля. Вони роблять кроки назад, коли раптово починається обвал, і вони швидко повертаються до виходу.''', reply_markup=arc_story_var9())
